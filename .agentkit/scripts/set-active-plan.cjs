...
if (geminiValidationOnly) {
  // Gemini consumes .agents/skills directly, but this adapter intentionally
  // ships no Claude/Codex hook runtime. Keep plan selection useful and honest:
  // validate below, but never claim session persistence without a state bridge.
  projectRoot = canonicalProjectRoot(installedProjectRoot);
} else {
  for (const root of candidateProjectRoots()) {
    if (opencodeValidationOnly) {
      projectRoot = canonicalProjectRoot(installedProjectRoot);
      break;
    }
...
if (!sessionContext && opencodeValidationOnly) {
  console.warn('Warning: OpenCode adapter has no reviewed session-state bridge; active plan state will not persist.');
