...
- **Grok Build**: skills auto-discovered from the project `.grok/skills/`
  registry and task agents from `.grok/agents/`. Grok derives native slash
  names from the directory name, so AgentKit skills are invoked as
  `/ak-plan`, `/ak-cook`, and `/ak-test` (not `/ak:plan`). The optional
  `.grok/workflows/agentkit-audit.rhai` surface composes these skills with
  bounded read-only fan-out; invoke it explicitly with
  `/workflow agentkit-audit {"task":"..."}`. It is not the skill registry and
  is never required for skill discovery. Grok does not need `.claude/`
  compatibility files to discover the native skills.
