...
function detectPlan(filePath) {
  // Preserve POSIX paths supplied by callers even when running on Windows;
  // this keeps serialized navigation state portable across runtimes.
  const pathApi = filePath.startsWith('/') ? path.posix : path;
  const dir = pathApi.dirname(filePath);
  const planFile = pathApi.join(dir, 'plan.md');
  ...
  return { isPlan: true, planDir: portable(dir), planFile: portable(planFile), phases: phases.map(f => portable(pathApi.join(dir, f))) };
}
