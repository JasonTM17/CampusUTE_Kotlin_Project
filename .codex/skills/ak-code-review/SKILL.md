...
argument-hint: "[#PR | COMMIT | --pending | codebase [parallel]] [--ultra]"
metadata:
  author: agentkit
  version: "2.1.0"
---
...
## Ultra review mode (`--ultra`)

Freeze one base/diff/evidence packet and run exactly five independent read-only
reviews against that exact digest. Reviewers must not see peer findings or
self-approve. The verifier may choose one unchanged review result or reject all;
it may not blend text or silently lower severity. It may build an index of
verbatim findings from candidate reports, with source candidate IDs, but may
not merge or paraphrase them. Preserve every accepted finding's evidence and
severity, and return `INCONCLUSIVE` when fewer than three
usable reviews remain. Apply the shared `ak.ultra/v1` contract and hard vetoes in
`ak-common/references/ultra-verifier-contract.md`. The controller owns final
verification and any repair; no candidate may edit, commit, push, or publish.
Without `--ultra`, all existing review modes and costs remain unchanged. Unknown
or incompatible flags stop with help.
