...
argument-hint: "[prompt-or-url] [--html] [--md] [--wiki] [--github] [--agent] [--ultra]"
license: MIT
metadata:
  author: agentkit
  version: "1.3.0"
...
| `--ultra` | After one contract-locking interview, run exactly five independent read-only advisory analyses on one frozen evidence packet and verify them with `ak.ultra/v1`. |

Flags combine freely. With no flags, deliver the advice in the conversation only.

### Ultra advisory mode (`--ultra`)

Interview the user exactly once per required question until the outcome contract
is locked; do not turn Ultra into five interviews. Then give exactly five
independent advisory candidates the same immutable evidence digest. Candidates
cannot see one another or implement the advice. The verifier accepts one
unchanged candidate, returns `REJECT_ALL`, or returns `INCONCLUSIVE` when fewer
than three usable candidates remain. It may redispatch one failed candidate and
must apply the shared hard-veto/rubric rules in
`ak-common/references/ultra-verifier-contract.md`. The default path and all
existing output flags remain unchanged when `--ultra` is absent.
