...
class DockerfileAnalyzer:
    ...
    def analyze_base_image(self) -> None:
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            if line.startswith('FROM'):
                # Parse the image token (excluding an optional stage alias) and
                # require either an explicit tag or an immutable digest.
                image_ref = line[4:].strip().split()[0] if line[4:].strip() else ''
                has_tag = '@' in image_ref or ':' in image_ref.rsplit('/', 1)[-1]
                if ':latest' in image_ref or not has_tag:
                    self.issues.append({
                        'line': i,
                        'severity': 'warning',
                        'category': 'base_image',
            ...
            if has_build_tools:
                self.issues.append({
                    ...
                    'category': 'optimization',
                        'message': 'Single-stage build with build tools; consider a multi-stage build',
    ...
    def analyze_combine_run(self) -> None:
        ...
        # Flush a run sequence that reaches EOF (there is no following
        # instruction to trigger the loop's normal boundary handling).
        if consecutive_runs > 1:
            self.suggestions.append({
                'line': first_run_line,
                'category': 'layers',
