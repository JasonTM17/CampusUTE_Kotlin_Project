...
argument-hint: "[context] [--ultra] OR ui [url]"
metadata:
  author: agentkit
  version: "1.1.0"
---
...
## Ultra strategy mode (`--ultra`)

Generate exactly five independent read-only test strategies or diagnostic
proposals from one frozen evidence packet. The candidates do not run tests,
see peer output, or self-approve. The controller remains the only actor that
executes commands and decides PASS/FAIL. Use
`ak-common/references/ultra-verifier-contract.md` for the digest, one-
redispatch bound, hard vetoes, and `ACCEPT`/`REJECT_ALL`/`INCONCLUSIVE` outcomes.
Without `--ultra`, keep the existing test execution and reporting path
unchanged; unknown flags must produce help.
The `ui` operation does not accept `--ultra`; reject that combination (and any
other unsupported combination) with concise help before running a test.
