...
async function main() {
  ...
  try {
    ...
    timer.end({ status: 'block', exit: 2, note: 'json-parse-failed', error: e.message });
    process.exit(2); // Invalid JSON cannot prove the path is safe
  }

  const { tool_input: toolInput, tool_name: toolName } = hookData;
  if (!toolInput || typeof toolInput !== 'object' || Array.isArray(toolInput) || Object.keys(toolInput).length === 0) {
